          ...
          <div class="fly-panel">
            ...
            <ul class="jie-row">
              <li v-for="(item,index) in postList" :key="'postlist' + index">
                ...
                <router-link class="jie-title link" :to="{name:'detail', params: {tid: item._id}}">{{item.title}}
                </router-link>
                <i>{{item.created | moment}}</i>
                <em class="layui-hide-xs">{{item.reads}}阅/{{item.answer}}答</em>
              </li>
              <div v-show="postList.length === 0" class="fly-none"
        ...
        <div class="layui-col-md6 fly-home-da">
          <div class="fly-panel">
            ...
            <ul class="home-jieda">
              ...
              <div v-show="commentList.length === 0" class="fly-none"
                style="min-height: 50px; padding:30px 0; height:auto;">
                <span>没有回答任何问题</span>
              </div>
            </ul>
