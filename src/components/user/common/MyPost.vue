<template>
    <div>
      <div class="overflow">
        <!-- H5不支持cellspacing&cellpadding CSS等价border-collapse:collapse border-spacing:0 -->
        <table class="layui-table" border="0">
          <thead>
            <tr class="layui-table-header">
              <th>
                <div class="layui-table-cell pl0">
                  <span>帖子标题</span>
                </div>
              </th>
              <th>
                <div class="layui-table-cell">
                  <span>状态</span>
                </div>
              </th>
              <th>
                <div class="layui-table-cell">
                  <span>结帖</span>
                </div>
              </th>
              <th>
                <div class="layui-table-cell">
                  <span>发表时间</span>
                </div>
              </th>
              <th>
                <div class="layui-table-cell">
                  <span>数据</span>
                </div>
              </th>
              <th class="min-cell">
                <div class="layui-table-cell">
                  <span>操作</span>
                </div>
              </th>
            </tr>
          </thead>

          <tbody>
            <tr class="text-center" v-for="(item, index) in lists" :key="'post' + index">
              <td>
                <router-link class="link" :to="{name: 'detail', params:{tid: item._id}}">{{ item.title }}</router-link>
              </td>
              <td>{{ item.status === '0' ? '打开' : '关闭' }}</td>
              <td :class="{'success': item.isEnd !=='0'}">
                {{ item.isEnd === '0' ? '未结' : '已结帖' }}
              </td>
              <td>{{ item.created | moment }}</td>
              <td>{{ item.reads }}阅/{{ item.answer }}答</td>
              <td>
                <div :class="{'layui-btn-disabled': item.isEnd === '1'}" @click="editPost(item)" class="layui-btn layui-btn-xs">编辑</div>
                <div @click="deletePost(item)" class="layui-btn layui-btn-xs layui-btn-danger">删除</div>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <pagination
      v-show="total > 1"
      :total="total"
      :current="current"
      :hasTotal="true"
      :hasSelect="true"
      @changeCurrent="handleChange"></pagination>
    </div>
</template>

<script>
import { getPostListByUid, deletePostByUid } from '../../../api/user'
import Pagination from '../../modules/pagination/Index'

export default {
  name: 'mypost',
  components: {
    Pagination
  },
  data () {
    return {
      lists: [],
      page: 0,
      limit: 10,
      total: 0,
      current: 0
    }
  },

  mounted () {
    this.getPostList()
  },

  methods: {
    // 获取用户发帖记录
    getPostList () {
      getPostListByUid({
        page: this.current,
        limit: this.limit
      }).then((res) => {
        // console.log('collectpost:', res)
        if (res.code === 200) {
          this.lists = res.data
          this.total = res.total
        }
      })
    },

    // 删除帖子
    deletePost (item) {
      this.$confirm('确定删除吗？', () => {
        // console.log('删除: ', item)
        if (item.isEnd !== '0') {
          this.$pop('shake', '帖子已结帖,无法删除')
          return
        }
        deletePostByUid({
          tid: item._id
        }).then((res) => {
          if (res.code === 200) {
            this.$pop('', '删除成功')
            this.lists.splice(this.lists.indexOf(item), 1)
          } else {
            this.$pop('shake', res.msg)
          }
        })
      }, () => {})
    },

    handleChange (val) {
      this.current = val // 修改页码后重新请求记录
      this.getPostList()
    },

    editPost (item) {
      if (item.isEnd === '1') {
        this.$pop('shake', '帖子已经结帖,无法编辑')
      } else {
        this.$router.push({
          name: 'edit',
          params: { tid: item._id, page: item }
        })
      }
    }
  }
}
</script>

<style lang='scss' scoped>
.overflow {
  overflow-y: auto;
}
.min-cell {
  min-width: 80px;
}
.link {
  color: #01aaed;
}
.success {
  color: #5FB878;
}
.layui-table-header {
  th {
    text-align: center;
    &:first-child {
      text-align: left;
    }
  }
}
</style>
