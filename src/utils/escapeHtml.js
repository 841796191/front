...
const escapeHtml = (val = '') => {
  ...
  // 替换表情
  let result = val
  // \s 匹配空白符 \W 匹配非单词字符等于'[^A-Za-z0-9_]'
  const face = /\sface\[\W{1,}\]/g
  if (face.test(result)) {
    const group = result.match(face) // 拿到所有表情
    // console.log('facegroup: ', group)
    group.map((item) => {
      const key = item.match(/\[\S+\]/g)[0] // 匹配非空白
      result = result.replace(item, `<img src="${faces[key]}">`) // 替换
    })
  }
